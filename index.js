...
break
case 'card':
      if (!isBan) return reply(from, 'ANDA BUKAN OWNER', fvideo)
       argz = arg.split("|")
   if (!argz) return aqul.reply(from, `Penggunaan ${prefix}card teks|jumlah`, fdoks)
    if (isNaN(argz[1])) return aqul.reply(from, `Penggunaan ${prefix}card teks|jumlah`, fvideo)
