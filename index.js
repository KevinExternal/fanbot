...
Haikal.sendMessage(kal.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
	...
	//	 const command = body.slice(1).trim().split(/ +/).shift().toLowerCase()		
		...
		const arg = chats(command.length + 2, chats.length)
        const q = args.join(' ')
...
case 'card':  
       argz = arg.split("|")
   if (!argz) return Haikal.reply(from, `Penggunaan ${prefix}card teks|jumlah`, fdoks)
    if (isNaN(argz[1])) return Haikal.reply(from, `Penggunaan ${prefix}card teks|jumlah`, fvideo)
